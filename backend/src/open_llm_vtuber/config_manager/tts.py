...
class ZipvoiceTTSConfig(I18nMixin):
    """Configuration for ZipVoice zero-shot voice-cloning TTS (sherpa-onnx)."""

    model_dir: str = Field(
        "./models/sherpa-onnx-zipvoice-distill-int8-zh-en-emilia", alias="model_dir"
    )
    vocoder: str = Field("./models/vocos_24khz.onnx", alias="vocoder")
    voice_dir: str = Field("./tts_voices", alias="voice_dir")
    default_voice: str = Field("露露", alias="default_voice")
    num_steps: int = Field(4, alias="num_steps")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "voice_dir": Description(
            en="Voice library directory (each voice = <name>/voice.wav + voice.json)",
            zh="声线库目录（每条声线 = <名字>/voice.wav + voice.json）",
        ),
        "default_voice": Description(
            en="Default voice name", zh="默认声线名称"
        ),
        "num_steps": Description(
            en="Flow-matching steps (4 recommended for distilled model)",
            zh="流匹配步数（蒸馏版推荐 4）",
        ),
    }
...
class TTSConfig(I18nMixin):
    ...
    zipvoice_tts: Optional[ZipvoiceTTSConfig] = Field(None, alias="zipvoice_tts")
