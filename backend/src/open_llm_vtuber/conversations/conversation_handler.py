...
async def handle_conversation_trigger(
    ...
    if msg_type == "ai-speak-signal":
    ...
    elif msg_type == "ai-reminder-signal":
        # 定时提醒到点：后端调度器触发，以用户消息形式进入对话流，
        # AI 会以自己的人格自然地把提醒说出来（不存历史，避免污染聊天记录）
        reminder_text = (data.get("text") or "").strip()
        user_input = (
            f"【定时提醒到点】设定的提醒时间到了。请以你的人格自然地提醒玩家：{reminder_text}"
            "（可以调侃可以关心，但要说清楚提醒的事项，这条指令本身不要复述）。"
        )
        metadata = {
            "proactive_speak": True,
            "skip_memory": True,
        ...
        logger.info(f"⏰ 定时提醒触发: {reminder_text}")
        await websocket.send_text(
            json.dumps({"type": "full-text", "text": "Reminder triggered..."})
        )
