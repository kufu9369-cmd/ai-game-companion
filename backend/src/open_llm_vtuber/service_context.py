...
class ServiceContext:
    ...
    async def handle_model_switch(self, websocket: WebSocket, model_name: str) -> None:
        """只切换 Live2D 形象（外貌），保持当前人设/对话上下文不变。

        与 handle_config_switch 的区别：不加载新配置、不重置 agent，
        只更新 live2d_model 的 model_info 并通知前端重新加载模型。
        """
        try:
            self.live2d_model.set_model(model_name)
            await websocket.send_text(
                json.dumps(
                    {
                        "type": "set-model-and-conf",
            ...
            await websocket.send_text(
                json.dumps(
                    {"type": "error", "message": f"Error switching model: {str(e)}"}
                )
            )
