...
def store_message(
    ...
    # 防复读护栏：AI 回复与历史末尾的 AI 回复高度相似时，不入库
    # （避免重复内容进入上下文 → 模型越抄越狠的恶性循环）
    if role == "ai" and history_data:
        import re as _re
        ...
        for prev in reversed(history_data):
            if prev.get("role") == "ai":
                a, b = _norm(prev.get("content", "")), _norm(content)
                if a and b and min(len(a), len(b)) >= 8:
                    ratio = _SM(None, a, b).ratio()
                    if a == b or ratio >= 0.72 or a in b or b in a:
                        logger.warning(
                            f"检测到与上一条 AI 回复高度相似（相似度 {ratio:.2f}），"
                            "跳过存储（防复读）"
                        )
                        return
                break  # 只看最近一条 AI 消息
...
def get_history_list(conf_uid: str) -> List[dict]:
    ...
    try:
        ...
        # 注意：这里只清理，不做批量删除失控。
        # 之前每次前端连上都会触发 history-list-request，本函数会把当前 conf 目录下所有
        # 空历史文件一次性 os.remove 掉；当累积的空文件 >=50 个时，会触发宿主环境的
        # 批量删除保护（SAFE_DELETE_BULK_CONFIRM_REQUIRED → SystemExit），直接把 WS
        # 处理协程打断，表现为前端 "connection closed (1006)" 反复掉线。
        # 因此限制单次清理上限，避免一次性大量删除。
        MAX_CLEAN_PER_CALL = 5
        if len(empty_history_uids) > 0 and len(os.listdir(conf_dir)) > 1:
            for uid in empty_history_uids[:MAX_CLEAN_PER_CALL]:
                try:
